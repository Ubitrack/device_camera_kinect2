...
void Kinect20ImageComponent::sendData(Measurement::Timestamp ts)
{
	...
	if (m_imagePortGray.isConnected()) {
		...
		switch (bodyID){
		case 0:
			pGrayImage = pImage->CvtColor(CV_BGRA2GRAY, 1);
...
boost::shared_ptr<Vision::Image> Kinect20ImageComponent::getDepthImage(bool createUVMap, boost::shared_ptr< Vision::Image >& uvMap) {
	...
	if (SUCCEEDED(hr))
	{
		img1 = cv::Mat(cv::Size(width, height), cv::Mat::MAGIC_VAL + CV_MAKE_TYPE(IPL2CV_DEPTH(16), 1), depthBuffer, cv::Mat::AUTO_STEP);
		// gpu init here
...
boost::shared_ptr<Vision::Image> Kinect20ImageComponent::getIRImage()
{
	...
	if (SUCCEEDED(hr))
	{
		img1 = cv::Mat(cv::Size(width, height), cv::Mat::MAGIC_VAL + CV_MAKE_TYPE(IPL2CV_DEPTH(16), 1), infraredBuffer, cv::Mat::AUTO_STEP);
		// gpu init here
